...
for (i in 1:6) {
	...
	for (j in 1:6) {
		...
		if (j == i) { 
			...
			smoothScatter(all.eff[, c(j + 1, i + 1)], axes = FALSE, xlab = "", ylab = "", cex = 0.002, pch = 16, xlim = c(-0.25, 0.25), ylim = c(-0.25, 0.25))
			if (j == 1) { axis(side = 2, cex.axis = 6/par("ps")/par("cex"), mgp = c(1.5, 0.3, 0), at = seq(-0.2, 0.2, 0.2), lwd = 0.5) }
				if (i == 6) { axis(side = 1, cex.axis = 6/par("ps")/par("cex"), mgp = c(1.5, -0.1, 0), at = seq(-0.2, 0.2, 0.2), lwd = 0.5) }
			text(-0.15, 0.2, paste("r = ", formatC(cor(all.eff[, c(i + 1, j + 1)], method = "spearman")[1, 2], format = "f", digit = 2)))
