...
sex <- args[7]
tc.min <- as.numeric(args[8])
tc.max <- as.numeric(args[9])
output.file <- args[10]
...
for (i in 1:nrow(result)) {
  ...
  if (sum(cov11 >= tc.min & cov11 <= tc.max) == length(cov11) & sum(cov12 >= tc.min & cov12 <= tc.max) == length(cov12) &
    ...
    if (freq.out1[i, 1] == "X" & sex == "male") {
    
      var <- sum(w11^2 * p11*(1-p11) * (1/nchr11*2 + 1/cov11)) + sum(w12^2 * p12*(1-p12) * (1/nchr12*2 + 1/cov12)) +
